...
def train():
    ...
    for epoch in range(EPOCH):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint_path = OUTPUT_DIR / f"clip_sneaker_best_fixed.pt"
            torch.save({
                'epoch': epoch,
    ...
    # Save final model
    final_path = OUTPUT_DIR / f"clip_sneaker_final_fixed.pt"
    torch.save({
        'epoch': EPOCH,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == "__main__":
    print("="*70)
    print("CLIP Fine-tuning for Sneakers (FIXED VERSION)")
    print("="*70)
