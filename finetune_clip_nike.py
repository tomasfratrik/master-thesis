...
def train():
    ...
    for epoch in range(EPOCH):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint_path = OUTPUT_DIR / f"clip_nike_best.pt"
            torch.save({
                'epoch': epoch,
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = OUTPUT_DIR / f"clip_nike_epoch_{epoch+1}.pt"
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    # Save final model
    final_path = OUTPUT_DIR / f"clip_nike_final.pt"
    torch.save({
        'epoch': EPOCH,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == "__main__":
    print("="*50)
    print("CLIP Fine-tuning for Nike Sneakers")
    print("="*50)
